...
struct Game {
    ...
    var identifierPlayer = 0
        
    mutating func deal3Cards() {
                
        if setCards.isEmpty && !findAllSetsOnTheTable().isEmpty {
            score[identifierPlayer] -= Constants.penaltyForDisMatchOrWrongDeal
        }
        ...
        if !matchedCards.contains(card) {
            ...
            if selectedCards.count == 3 {
                if isSet(in: selectedCards) {
                    ...
                    score[identifierPlayer] += max(result, Constants.minScoreForMatch)
...
//        let buffer = cardsOnTheTable.map { (cardOnTable) -> Card in
//            if setCards.contains(cardOnTable) {
//                if let card = getCard() {
//                    return card
//                }
//            } else {
